...
const HomePage = () => {
    ...
    const fetchLatestBlogs = async (page = 1) => {
        setLoading(true);
        try {
            const response = await axios.post('http://localhost:3000/latest-blogs', { page });
...
//            setLoading(false);
 ...
 const fetchBlogsByCategory = async ({ page = 1 }) => {
    ...
    try {
        ...
        const response = await axios.post('http://localhost:3000/search-blogs', { tag: pageState, page });
        const data = response.data;
